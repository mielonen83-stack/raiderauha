...
st.set_page_config(
    page_title="Raiderauha – Reaaliaikainen Junatutka, Aikataulut & Rataliikennehäiriöt",
    page_icon="🚆",
    layout="wide",
)
...
def hae_raportit(juna_numero):
    ...
    kursori = yhteys.cursor()
    kursori.execute(
        "SELECT raportti, aika FROM raportit WHERE juna_numero = ? ORDER BY id DESC",
        (str(juna_numero),),
    )
...
st.title("🚆 Raiderauha")
st.markdown(
    "##### *Reaaliaikainen junatutka, tekoälyn rauha-alueet, sää ja matkustajien live-raportit*"
)
st.divider()

# --- MATKABINGO-OSIO AINA NÄKYVISSÄ ---
st.markdown("### 🎫 Junamatkustajan Matkabingo")
st.markdown("*Bongaa klassisia junailmiöitä matkan varrelta ja rukskaa ruutuja!*")
...
if st.session_state.haku_tehty:
    st.markdown(
        f"### 🗺️ Reitti: **{valittu_lahto_nimi}** ➔ **{valittu_paikka_nimi}** ({valittu_pvm.strftime('%d.%m.%Y')})"
    )
    ...
    if p_lat and p_lon:
        try:
            ...
            st.success(
                f"🌤️ **Sää määränpäässä ({valittu_paikka_nimi.split(' ')[0]}):** {lampo}°C"
            )
        except:
            pass
    ...
    if vastaus.status_code == 200:
        ...
        if not isinstance(junat, list) or not junat:
            st.warning(
                "⚠️ Valitsemallesi päivälle ja välille ei löytynyt suoria junavuoroja."
            )
        else:
            aktiiviset_junat = []
            ...
            if not aktiiviset_junat:
                st.warning(
                    "⚠️ Valitsemallesi päivälle ei löytynyt enää aktiivisia/tulevia vuoroja tällä välillä."
                )
            else:
                st.success(
                ...
                for juna in aktiiviset_junat:
                    ...
                    with st.expander(
                        f"🚆 {t_tyyppi} {t_num} | Lähtö klo {juna['lahto']} ➔ Perillä klo {juna['saapuminen']} ({status_teksti})"
                    ):
                        ...
                        if asemat_matkalla:
                            for asema_info in asemat_matkalla:
                                ...
                                st.write(
                                    f"{tila_emoji} **{kokonainen_nimi}** – klo {asema_info['aika']}{myoha_str}"
                                )
                        ...
                        if viimeinen_asema and viimeinen_asema["myohassa"] > 0:
                            st.warning(
                                f"⚠️ **Vaihdot vaarassa:** Juna on noin {viimeinen_asema['myohassa']} min myöhässä perille saapuessa."
                            )
                        else:
                            st.success(
                        ...
                        if tallennetut_raportit:
                            for r_teksti, r_aika in tallennetut_raportit:
                                st.write(
                                    f"💬 *\"{r_teksti}\"* — <small style='color: gray;'>({r_aika})</small>",
    ...
    st.info(
        "👈 Valitse asemat ja matkustuspäivä sivupalkista, ja klikkaa **Etsi junat ja Rauhavahti**."
    )
